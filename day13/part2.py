...
def main():
    ...
    width = get_max_width(dots)
    height = get_max_height(dots)
    visualize_dots(dots, width, height, empty=' ')
...
def visualize_dots(dots, width, height, empty='.'):
    for y in range(height):
        for x in range(width):
            if (x, y) in dots:
                print("#", end="")
...
def get_max_width(dots):
    return max(dots, key=lambda coord: coord[0])[0] + 1


def get_max_height(dots):
    return max(dots, key=lambda coord: coord[1])[1] + 1
